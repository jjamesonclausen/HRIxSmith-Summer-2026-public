---
Created: 2026-07-03
Updated: 2026-07-07
Sources: [[va11]]
Source_count: 1
Tags: #methods
---
## Particle Image Velocimetry (PIV) Testing

Experimental flow-visualization method that tracks seeded particles in an illuminated plane to recover wake velocity and vortex structure. (source: sources/va11.md)

![Source figure](../../images/va11-fig16.jpg)
Original caption: Fig. 16. PIV test of near wake of a VAWT [19]. [[va11|Source]]
![Source figure](../../images/va11-fig17.jpg)
Original caption: Fig. 17. Contours of normalized out-of-plane vorticity for the combined field of views of the horizontal plane [*] [19]. [[va11|Source]]

- The review treats PIV as a main wake-analysis method for H-rotor VAWTs. (source: sources/va11.md)
- It describes planar PIV for the blade mid-span wake and stereoscopic PIV for vertical planes parallel to the freestream. (source: sources/va11.md)
- In the reviewed studies, PIV was used to observe vortex shedding, roll-up, release, blade-wake interaction, wake asymmetry, and wake transition. (source: sources/va11.md)
- The paper also cites PIV data as a validation target for CFD wake predictions. (source: sources/va11.md)

Related:
- [[Wind Tunnel Testing]]
- [[3D Particle Tracking Velocimetry]]
- [[CFD and Validation]]
- [[H-rotor Wake Aerodynamics]]

#methods
