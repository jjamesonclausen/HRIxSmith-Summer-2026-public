...
We did not begin this project with a finished AI research assistant. We began with no prior knowledge on vertical axis wind turbines, and were working with unfamiliar tools. This project's goals have been twofold: to design a VAWT, but also to learn and document where and how agentic tools can assist in the learning and designing process. In this process we first learned how to use new tools such as Opencode, VScode, Obsidian, and GitHub. The first week was therefore not spent researching VAWTs but rather experimenting and learning about these new tools.  
We completed a short workshop for creating a LLM wiki, learned enough GitHub to work in one shared repository, and designed baseline rules for the wiki to fine-tune it for our project. We continued to add to and refine these rules throughout the project, ensuring the LLM relied solely on project sources, would cite its claims, and would label unsupported material. We built an evaluation loop with questions made to intentionally target common fail points of LLMs such as unfounded claims, hallucinations, or sycophancy to test the LLM instead of assuming that an answer which sounded right was reliable. We wanted not to hand off our judgment to an LLM, but to create a source-grounded workspace that could help us find and sort information, locate gaps, and answer technical questions.  

The real setup work came when we started feeding research into the system. We created a workflow to convert PDFs of research papers we gathered into Markdown files while preserving figures and equations. We defined the organization of our wiki pages around concepts, methods, designs, and parameters to create a well-structured database of reliable information. We selected valuable parameters---such as cut-in speed, efficiency, and rated speed--and had the LLM generate metadata for our wiki's design pages to better sort and rank designs within Obsidian. (if we include pictures lets put one of the metadata ) We had to respond to problems like missing images, poor formatting, unwanted edits to source text, slow or inconsistent runs, and broken links. We addressed these problems by tightening the defined skills, splitting detailed procedures into separate schema documents, and adding Python-based image extractions. All of these efforts went toward creating a tool that can synthesize material quickly and generate fast summarizations of complex problems. The most time-consuming part was not actually asking questions about turbines. It was creating a source we could trust. We have found, however, that with this tool we must be more intentional with our goal of learning, as it can be easy to use it as an answering machine rather than a teacher. It is tempting to skip the slower learning that engineering requires. The value of the wiki has come from treating it as an evolving research process and teaching tool, not as an expert whose first answer should be always be accepted. Since it has been trained on only material we sourced, it has become a much more reliable and refined resource than a general chatbot like Chat GPT or Gemini could be for this project. 

***** i need to add some more specific info on what's in agents and maybe more on the schema pages 
