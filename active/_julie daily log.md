...
- Start learning fluid mechanics - 9:30 ( 4hr)
...
- Stayed on with anna and worked on combining forces
  - Figured out git hub and working asynchronous
...
- Watched youtube videos to learn about git hub
- Call with mike to give update - 3:30
- Stay on call to continue working with git hub
...
- Images - we are not having it make figure captions but seems like this might be more complicated than its worth - do go check that images are being used well in the concept pages
  - Consider the organization of these images - can we have the llm generate tags for them - which tags would be use full Personal thoughts
...
- Download and mess around with zoo keeper - 4 hr
  ...
  - Meeting with whole team - 1 hr
    - + took some notes on meeting - 0.5 hr

**Notes from meeting:**

To-Dos
1. Finalize the VAWT design goal
- Define exactly what we are trying to design.
- Determine success criteria for the project.
- Decide whether location assumptions (such as using the Boston area wind profile) will be included.
2. Create a draft project work plan
- Generate 3-5 AI-created project plans with different assumptions.
- Compare plans based on:
  - timeline
  - available tools
  - desired deliverables
  - available personnel
- Combine the best parts of each into one final plan.
3. Test CSV to JSON conversion
- Convert existing metadata CSV files into JSON.
- Run the wiki ingestion pipeline again.
- Compare:
  - metadata completeness
  - metadata accuracy
  - consistency of extraction
- Determine whether JSON solves the metadata issue.
4. Build reusable prompt and planning assets Create:
- /plans folder
- /prompts folder
- Markdown files containing:
  ...
  - desired outputs
  - prompting templates
5. Explore Zookeeper CAD workflow
- Read documentation.
- Learn best prompting practices.
- Test: source-code interface, GUI interface, agentic editor
  - Determine which interface works best for each task. Deliverables for Next Meeting
- Final design objective
- Draft work plan created from multiple AI-generated options
- Results of JSON metadata testing
- Initial Zookeeper exploration report
- Initial SimScale exploration report
